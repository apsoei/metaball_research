...
// std::cout<< split.size() <<std::endl;
    ...
    {
        std::cout<< "SOMETHING'S WRONG G G G G GG GG GGG G G" <<std::endl;
        return plgs;
    }
    while(plgs.size()<total)
    {
        if(plgs.size()==0 && split.size()==3)
        {
// std::cout<< __FUNCTION__ << "-+-+-+-+-+-+-+-+-+" << __LINE__<<std::endl;
            std::vector<int> tempPlg;
            for(int spl : split)
            {
                tempPlg.push_back(spl);
            }
            plgs.push_back(tempPlg);
        }
        else
        {
// std::cout<< __FUNCTION__ << "-+-+-+-+-+-+-+-+-+" << __LINE__<<std::endl;
...
// std::cout<< __FUNCTION__ << "-+-+-+-+-+-+-+-+-+" << __LINE__<<std::endl;
                        ...
                        tempPlg.push_back(src);
                        if(src!= split[0] && src!= split[1])
                        {
                            usedNotItsc.push_back(src);
                        }
// std::cout<<tempPlg.size()<<std::endl;
                    }
                    else if(tempPlg.size()==0 && plgs.size()==0 && (src==split[0] || src==split[1]) )
                    {
// std::cout<< __FUNCTION__ << "-+-+-+-+-+-+-+-+-+" << __LINE__<<std::endl;
                        tempPlg.push_back(src);
                        caseA = true;
...
// std::cout<< __FUNCTION__ << "-+-+-+-+-+-+-+-+-+" << __LINE__<<std::endl;
                    ...
                    {
                        plgs.push_back(tempPlg);
// std::cout<< "individual polygon size: " << tempPlg.size() <<std::endl;
                        break;
                    }
                }
            }
            ...
            {
                for(int i = 0; i<srcPlg.size(); ++i)
                {
                    if(srcPlg[i]!=split[0] && srcPlg[i]!=split[1] && srcPlg[i]!=split[2])
                    {
                        bool used = false;
                        for(int u : usedNotItsc)
                        {
                        ...
                        {
// std::cout<< __FUNCTION__ << "-+-+-+-+-+-+-+-+-+" << __LINE__<<std::endl;
                            usedNotItsc.push_back(srcPlg[i]);
                            if(i<1)
                            {
                                tempPlg.push_back(srcPlg.back());
                                tempPlg.push_back(srcPlg[i]);
                                tempPlg.push_back(srcPlg[i+1]);
                                break;
                            }
                            else if(i>=srcPlg.size()-1)
                            {
                                tempPlg.push_back(srcPlg[i-1]);
                                tempPlg.push_back(srcPlg[i]);
                                tempPlg.push_back(srcPlg[0]);
                                break;
                            }
                            else
                            {
...
std::cout << __FUNCTION__ << " ---- " <<__LINE__<< std::endl;
        ...
        // if(cube.itscs.size()==2)
        // {
            
        //     objVtx.push_back(cube.itscs[0].x);
        //     objVtx.push_back(cube.itscs[0].y);
        //     objVtx.push_back(cube.itscs[0].z);
        //     if(cube.itscs[0].vtident == 0)
        //     {
        //         cube.itscs[0].vtident = vtIdent;
        //         vtIdent++;
        //     }
        //     objVtx.push_back(cube.itscs[1].x);
        //     objVtx.push_back(cube.itscs[1].y);
        //     objVtx.push_back(cube.itscs[1].z);
        //     if(cube.itscs[1].vtident == 0)
        //     {
        //         cube.itscs[1].vtident = vtIdent;
        //         vtIdent++;
        //     }

        //     tempFace.push_back(cube.itscs[0].vtident);
        //     tempFace.push_back(cube.itscs[1].vtident);
        //     tempFace.push_back(cube.itscs[0].vtident);
        //     tempFace.push_back(cube.itscs[1].vtident);
        // }
