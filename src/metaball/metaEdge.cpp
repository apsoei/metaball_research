...
// std::cout << a << std::endl;
    ...
    public:
        ...
        // front: edge  0,2,9,4
        // back: edge   5,7,11,6
        std::vector<float> itscIn;
        std::vector<float> itscOut;
        enum ITSC
        {
            NONE,
    ...
    std::vector<float> getPlaneEqn(Cube::Edge ed1,Cube::Edge ed2);

    std::vector<float> getIntersection(std::vector<float> planEqnConst, std::vector<float> oneEdgePt, std::vector<float> edgeVec);
    
    // FOR TESTING . . .
    std::vector<float> itscTest;
    std::vector<float> itscTestCol;


    // FOR INPUT EDGE PROCESSING . . . < < < < <
};

std::vector<float> ApplicationMain::getPlaneEqn(Cube::Edge ed0,Cube::Edge ed1)
{
    // plane eqn: n_x * x + n_y * y + n_z * z = d
    // returns {n_x,n_y,n_z,d}
    ...
    std::vector<float> ptA;
    std::vector<float> ptB;
    std::vector<float> ptC;
    if(ed0.vtx1[0]==ed1.vtx2[0] && ed0.vtx1[1]==ed1.vtx2[1] && ed0.vtx1[2]==ed1.vtx2[2])
    {
        ptA.push_back(ed0.vtx1[0]); ptA.push_back(ed0.vtx1[1]); ptA.push_back(ed0.vtx1[2]);
        ptB.push_back(ed0.vtx2[0]); ptB.push_back(ed0.vtx2[1]); ptB.push_back(ed0.vtx2[2]);
        ptC.push_back(ed1.vtx1[0]); ptC.push_back(ed1.vtx1[1]); ptC.push_back(ed1.vtx1[2]);
    }
    else if(ed0.vtx1[0]==ed1.vtx1[0] && ed0.vtx1[1]==ed1.vtx1[1] && ed0.vtx1[2]==ed1.vtx1[2])
    {
        ptA.push_back(ed0.vtx1[0]); ptA.push_back(ed0.vtx1[1]); ptA.push_back(ed0.vtx1[2]);
        ptB.push_back(ed0.vtx2[0]); ptB.push_back(ed0.vtx2[1]); ptB.push_back(ed0.vtx2[2]);
        ptC.push_back(ed1.vtx2[0]); ptC.push_back(ed1.vtx2[1]); ptC.push_back(ed1.vtx2[2]);
    }
    else if(ed0.vtx2[0]==ed1.vtx1[0] && ed0.vtx2[1]==ed1.vtx1[1] && ed0.vtx2[2]==ed1.vtx1[2])
    {
        ptA.push_back(ed0.vtx2[0]); ptA.push_back(ed0.vtx2[1]); ptA.push_back(ed0.vtx2[2]);
        ptB.push_back(ed0.vtx1[0]); ptB.push_back(ed0.vtx1[1]); ptB.push_back(ed0.vtx1[2]);
        ptC.push_back(ed1.vtx2[0]); ptC.push_back(ed1.vtx2[1]); ptC.push_back(ed1.vtx2[2]);
    }
    else
    {
    ...
    std::vector<float> vec1 = {ptB[0]-ptA[0],ptB[1]-ptA[1],ptB[2]-ptA[2]};
    std::vector<float> vec2 = {ptC[0]-ptA[0],ptC[1]-ptA[1],ptC[2]-ptA[2]};
    auto normal = CrossProduct(vec1,vec2);
    auto d = (normal[0]*ptA[0] + normal[1]*ptA[1] + normal[2]*ptA[2]);
    normal.push_back(d);
    ...
    // auto ptC = ed1.vtx2;
    // std::vector<float> vec1 = {ptB[0]-ptA[0],ptB[1]-ptA[1],ptB[2]-ptA[2]};
    // std::vector<float> vec2 = {ptC[0]-ptB[0],ptC[1]-ptB[1],ptC[2]-ptB[2]};
    // auto normal = CrossProduct(vec1,vec2);
    // // dot prod between normal and ptA
    // auto d = -(normal[0]*ptA[0] + normal[1]*ptA[1] + normal[2]*ptA[2]);
    // normal.push_back(d);
...
std::vector<float> ApplicationMain::getIntersection(std::vector<float> planEqnConst, 
                                                    std::vector<float> oneEdgePt, std::vector<float> edgeVec)
{
    auto a = edgeVec[0]; auto b = edgeVec[1]; auto c = edgeVec[2];

    // plane eqn: n_x * x + n_y * y + n_z * z = d

    std::vector<float> intersection;
    auto xp = oneEdgePt[0];     auto yp = oneEdgePt[1];     auto zp = oneEdgePt[2];
    auto nx = planEqnConst[0];  auto ny = planEqnConst[1];  auto nz = planEqnConst[2]; auto d = planEqnConst[3];

    if(fabs(nx*a + ny+b + nz*c)<tol)
    {
        return intersection;
    }
    auto t = (d-nx*xp-ny*yp-nz*zp) / (nx*a + ny+b + nz*c);
    auto x = xp + a*t;
    auto y = yp + b*t;
    auto z = zp + c*t;
    auto toBeCompared = nx*x+ny*y+nz*z;
    // std::cout << "tobecompared = "<< toBeCompared<<std::endl;
    // std::cout << "plane d = " <<d <<std::endl;
    if(nx*x+ny*y+nz*z==d)
    {
...
std::cout << __FUNCTION__ << " ---- " <<__LINE__<< std::endl;
            ...
            // FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>>
            // FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>>
            // FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>> FOR PLANE - LINE INTERSECT >>>>>

            //                                         TOP , BOT , RIG, LEF  , FRO , BAC
            std::vector<std:: pair <int,int>> plEd = {{0,3},{8,9},{3,4},{1,6},{0,2},{5,7}};
            std::vector<Cube::ITSC> itscFace = {cube.TOP,cube.BOTTOM,cube.RIGHT,cube.LEFT,cube.FRONT,cube.BACK};
            
            
            for(int i = 0; i<plEd.size(); ++i)
            {                
                auto eqn = getPlaneEqn(edges[plEd[i].first],edges[plEd[i].second]);
    
                for(auto line : inputEdges)
                {
                    std::vector<float> pt = {line[0],line[1],line[2]};
                    std::vector<float> edVec = {line[3]-line[0],line[4]-line[1],line[5]-line[2]};
                    auto itsc = getIntersection(eqn,pt,edVec);
                    if(0 < itsc.size())
                    {
                        cube.itsc = itscFace[i];
                        if(0==cube.itscIn.size())
                        {
                            cube.itscIn.push_back(itsc[0]);
                            cube.itscIn.push_back(itsc[1]);
                            cube.itscIn.push_back(itsc[2]);
                            
                        }
                        else
                        {
                            cube.itscOut.push_back(itsc[0]);
                            cube.itscOut.push_back(itsc[1]);
                            cube.itscOut.push_back(itsc[2]);
                        }
                        // for test >>
                        itscTest.push_back(itsc[0]);
                        itscTest.push_back(itsc[1]);
                        itscTest.push_back(itsc[2]);
                        itscTestCol.push_back(1);itscTestCol.push_back(0);itscTestCol.push_back(0);itscTestCol.push_back(1);
                        
                    }
                    // if(cube.itscIn.size()>0 && cube.itscOut.size()>0)
                    // {
                    //     break;
                    // }
                }
            ...
            for(int i : idx)
            {
                ...
                {
                    // DEBUGGING . . . >>>>
// std::cout << "interped point: " << edges[i].interp[0] <<", "<< edges[i].interp[1]<<", "<< edges[i].interp[2] <<std::endl;
...
// std::cout <<"edge intersection point number:" << itscTest.size()/3 <<std::endl;
// for(int i = 0; i<51; ++i)
// {
//     std::cout << itscTest[i] <<" ,";
//     if(i%3==2)
...
void ApplicationMain::Draw(void) const
{
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
    glEnableClientState(GL_COLOR_ARRAY);
    glColorPointer(4,GL_FLOAT,0,itscTestCol.data());
    glVertexPointer(3,GL_FLOAT,0,itscTest.data());
    glPointSize(10);
    glDrawArrays(GL_POINTS,0,itscTest.size()/3);
    glDisableClientState(GL_VERTEX_ARRAY);
    glDisableClientState(GL_COLOR_ARRAY);
