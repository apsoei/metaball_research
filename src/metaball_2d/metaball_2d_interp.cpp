...
float ApplicationMain::square_dist(float x, float y, float ballx, float bally)
{
    ...
    float eps = 1e-4;       // Avoid division by zero
...
// std::cout<<__FUNCTION__<<__LINE__<<std::endl;
    ...
    {
        dist = (x-ballx)*(x-ballx) + (y-bally)*(y-bally);
    }
    return dist;
...
void ApplicationMain::update(std::vector<Cell> cells, std::vector<Ball> balls)
{
    ...
    for(auto &cell : cells)
    {
        ...
        {
            a+= (ball.rad*ball.rad) / 
                square_dist(cell.a[0],cell.a[1],ball.x,ball.y);
            
            b+= (ball.rad*ball.rad) / 
                square_dist(cell.b[0],cell.b[1],ball.x,ball.y);

            c+= (ball.rad*ball.rad) / 
                square_dist(cell.c[0],cell.c[1],ball.x,ball.y);

            d+= (ball.rad*ball.rad) / 
                square_dist(cell.d[0],cell.d[1],ball.x,ball.y);
        }
